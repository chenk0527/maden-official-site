...
export default function HomePage() {
  return (
    <>
      ...
      <section style={{ padding: '0 48px', background: '#230808', borderBottom: '1px solid rgba(232,57,26,.18)' }}>
        <div id="numbers-grid" style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', textAlign: 'center', padding: '56px 0' }}>
          {[
            ...
            <div key={item.label} className="r" style={{ borderRight: i < 3 ? '1px solid rgba(232,57,26,.22)' : 'none', padding: '0 24px' }}>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2.4rem, 5vw, 4rem)', fontWeight: 300, color: '#e8391a', lineHeight: 1, letterSpacing: '.05em' }}>
      ...
      <section id="about" style={{ padding: '140px 48px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '100px', alignItems: 'center' }}>
          <div className="r-left" style={{ position: 'relative', overflow: 'hidden', aspectRatio: '3/4' }}>
            ...
            />
            <div style={{ position: 'absolute', bottom: '24px', right: '24px', background: 'rgba(204,34,0,.9)', color: '#1c0505', fontFamily: "'Cormorant Garamond', serif", fontSize: '1rem', fontWeight: 600, letterSpacing: '.15em', padding: '10px 18px' }}>
              Since 2008
            </div>
          </div>
          ...
          <div className="r-right">
            ...
            <p style={{ fontSize: '1rem', lineHeight: 2, fontWeight: 400, color: 'rgba(245,237,232,.72)', marginBottom: '20px' }}>
              2008年，三位热爱美式复古文化的年轻人在重庆创立了马登。从一间小工作室到如今全渠道<strong style={{ color: '#e8391a', fontWeight: 400 }}>70+店铺</strong>、<strong style={{ color: '#e8391a', fontWeight: 400 }}>7家实体门店</strong>的品牌版图，马登始终只做一件事——<strong style={{ color: '#e8391a', fontWeight: 400 }}>最纯正的美式复古工装</strong>。
            </p>
            <p style={{ fontSize: '1rem', lineHeight: 2, fontWeight: 400, color: 'rgba(245,237,232,.72)', marginBottom: '20px' }}>
              每一件马登工装都承载着 Americana 精神：坚韧、务实、不妥协。我们从底特律工厂、德州牛仔、越战军服中汲取灵感，用当代工艺还原经典的美式穿着哲学。
            </p>
            <p style={{ fontSize: '1rem', lineHeight: 2, fontWeight: 400, color: 'rgba(245,237,232,.72)', marginBottom: '20px' }}>
              不追逐潮流，因为<strong style={{ color: '#e8391a', fontWeight: 400 }}>经典从不过时</strong>。
            </p>
            <div style={{ marginTop: '40px', fontSize: '.6rem', letterSpacing: '.4em', opacity: 0.3, display: 'flex', alignItems: 'center', gap: '20px' }}>
              <span style={{ width: '40px', height: '1px', background: 'rgba(204,34,0,.3)', display: 'inline-block' }} />
              重庆 · 2008年创立
            </div>
          </div>
        </div>
      ...
      {/* Philosophy */}
      <section id="philosophy" style={{ padding: '140px 48px', background: '#230808' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '48px', marginTop: '56px' }}>
            {[
              ...
              <div key={item.title} className="r">
                <div style={{ marginBottom: '20px', color: '#e8391a', opacity: 0.7 }}>
                  <item.Icon size={44} color="currentColor" strokeWidth={1.1} />
                </div>
                <h4 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.8rem', fontWeight: 400, letterSpacing: '.08em', marginBottom: '16px', color: '#e8391a' }}>{item.title}</h4>
                <p style={{ fontSize: '1rem', lineHeight: 1.9, fontWeight: 400, color: 'rgba(245,237,232,.7)' }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '80px 48px' }}>
        <div className="r" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.8rem, 4.5vw, 4rem)', fontWeight: 300, lineHeight: 1.4, letterSpacing: '.03em', maxWidth: '800px', color: 'rgba(245,237,232,.6)' }}>
          &ldquo;不追逐潮流，<br />因为<em style={{ fontStyle: 'italic', color: '#e8391a' }}>经典</em>从不过时。&rdquo;
        </div>
      </section>

      {/* Timeline */}
      <section id="history" style={{ padding: '140px 48px', background: '#230808' }}>
      ...
      {/* 旗下品牌 */}
      <section style={{ padding: '140px 48px', background: '#230808' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <p className="r" style={{ fontSize: '1rem', color: 'rgba(245,237,232,.45)', marginBottom: '64px', fontWeight: 400 }}>
            男鞋 · 男装 · 女装，一个集团下的三种表达。
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2px' }}>
            {brands.map((b, i) => (
              <div key={b.name} className="r" style={{
                background: '#260a0a', padding: '48px 36px', position: 'relative',
                borderTop: '1px solid rgba(232,57,26,.18)',
                transition: 'background .3s',
              }}>
                <span style={{
                  position: 'absolute', top: '20px', right: '24px',
                  fontFamily: "'Cormorant Garamond', serif",
                  fontSize: '3.5rem', fontWeight: 300, color: '#e8391a', opacity: 0.08, lineHeight: 1,
                }}>
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                  <p style={{ fontSize: '.6rem', letterSpacing: '.3em', color: '#e8391a', opacity: 0.6 }}>{b.tag}</p>
                  <span style={{ fontSize: '.55rem', letterSpacing: '.1em', color: '#e8391a', border: '1px solid rgba(232,57,26,.28)', padding: '3px 8px' }}>{b.badge}</span>
                </div>
                ...
                <p style={{ fontSize: '.6rem', letterSpacing: '.25em', color: 'rgba(245,237,232,.2)', marginBottom: '20px' }}>{b.en}</p>
                <p style={{ fontSize: '.95rem', lineHeight: 1.9, color: 'rgba(245,237,232,.58)', fontWeight: 400 }}>{b.desc}</p>
              </div>
            ))}
          </div>
          <div className="r" style={{ textAlign: 'center', marginTop: '48px' }}>
            <Link href="/about" style={{ fontSize: '.6rem', letterSpacing: '.4em', opacity: 0.4, borderBottom: '1px solid rgba(204,34,0,.3)', paddingBottom: '4px' }}>
              了解更多品牌故事 →
            </Link>
          </div>
        </div>
      ...
      <section style={{ padding: '140px 48px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <p className="r" style={{ fontSize: '1rem', color: 'rgba(245,237,232,.4)', maxWidth: '540px', margin: '0 auto 80px', lineHeight: 1.8, textAlign: 'center', fontWeight: 400 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '40px 32px' }}>
            {charities.map((c, i) => (
              <div key={c.year} className="r" style={{
                borderLeft: '2px solid rgba(232,57,26,.45)',
                paddingLeft: '20px',
                position: 'relative',
              }}>
                <div style={{
                  ...
                  background: '#e8391a', opacity: 0.6,
                }} />
                <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem', color: '#e8391a', opacity: 0.7, marginBottom: '8px', fontWeight: 300 }}>{c.year}</p>
                <h4 style={{ fontSize: '.95rem', fontWeight: 400, letterSpacing: '.03em', marginBottom: '8px', color: 'rgba(245,237,232,.85)' }}>{c.title}</h4>
                <p style={{ fontSize: '.85rem', lineHeight: 1.7, color: 'rgba(245,237,232,.42)', fontWeight: 400 }}>{c.desc}</p>
              </div>
            ))}
          </div>
      ...
      <section id="shop" style={{ padding: '140px 48px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1px', background: 'rgba(232,57,26,.14)' }}>
          {shopChannels.map(ch => (
      ...
      <style>{`
        .cta-btn-link {
          ...
          padding: 14px 48px;
          border: 1px solid rgba(232,57,26,.45);
          font-size: .65rem;
          letter-spacing: .4em;
          font-weight: 300;
          transition: all .6s;
