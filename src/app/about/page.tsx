  ...
  return (
    <>
      ...
      <section style={{ paddingTop: '180px', padding: '180px 48px 80px', textAlign: 'center' }}>
        ...
        <p className="r" style={{ fontSize: '1rem', fontWeight: 400, color: 'rgba(240,235,232,.55)', maxWidth: '500px', margin: '0 auto', lineHeight: 1.8 }}>
      ...
      <section style={{ padding: '80px 48px 140px' }}>
        <div className="about-split">
          ...
          <div className="r-right">
            ...
            <p style={{ fontSize: '1rem', lineHeight: 2, fontWeight: 400, color: 'rgba(240,235,232,.7)', marginBottom: '20px' }}>
            ...
            <p style={{ fontSize: '1rem', lineHeight: 2, fontWeight: 400, color: 'rgba(240,235,232,.7)' }}>
      ...
      {/* 旗下品牌 */}
      <section style={{ padding: '120px 48px', background: '#120808' }}>
            ...
            {brands.map((b, i) => (
              <div key={b.name} className="r brand-card" style={{
                ...
                <span style={{
                  ...
                  color: '#cc2200', opacity: 0.12, lineHeight: 1,
                  userSelect: 'none',
                }}>
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  <p style={{ fontSize: '.65rem', letterSpacing: '.3em', color: '#cc2200', opacity: 0.7, fontWeight: 400 }}>{b.tag}</p>
                  <span style={{ fontSize: '.6rem', letterSpacing: '.1em', color: '#cc2200', border: '1px solid rgba(204,34,0,.3)', padding: '3px 10px', fontWeight: 400 }}>{b.badge}</span>
                </div>
                <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.8rem', fontWeight: 300, letterSpacing: '.08em', marginBottom: '6px', color: 'rgba(240,235,232,.95)' }}>{b.name}</h3>
                <p style={{ fontSize: '.65rem', letterSpacing: '.25em', color: 'rgba(240,235,232,.25)', marginBottom: '20px', fontWeight: 400 }}>{b.en}</p>
                <p style={{ fontSize: '.95rem', lineHeight: 1.9, fontWeight: 400, color: 'rgba(240,235,232,.6)' }}>{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="phil-3col">
            {[
              ...
              <div key={item.title} className="r">
                <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.5rem', fontWeight: 400, letterSpacing: '.05em', marginBottom: '16px', color: '#cc2200' }}>{item.title}</h3>
                <p style={{ fontSize: '1rem', lineHeight: 1.9, fontWeight: 400, color: 'rgba(240,235,232,.65)' }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 公益足迹 */}
      <section style={{ padding: '120px 48px', background: '#120808', overflow: 'hidden' }}>
        <div style={{ maxWidth: '1300px', margin: '0 auto' }}>
          ...
          <p className="r" style={{ fontSize: '.7rem', letterSpacing: '.3em', color: '#cc2200', opacity: 0.5, marginBottom: '16px', textAlign: 'center', fontWeight: 400 }}>MADEN CHARITY FUND</p>
          <p className="r" style={{ fontSize: '1rem', fontWeight: 400, color: 'rgba(240,235,232,.45)', marginBottom: '80px', lineHeight: 1.8, textAlign: 'center', maxWidth: '600px', margin: '0 auto 80px' }}>
          ...
          <div style={{ position: 'relative' }}>
            {/* 中间横线 */}
            <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: '1px', background: 'rgba(204,34,0,.12)', transform: 'translateY(-50%)' }} />
                  ...
                  {c.row === 'top' ? (
                    ...
                    <>
                      <div className="charity-content-top">
                        <h4 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem', fontWeight: 400, color: 'rgba(240,235,232,.9)', marginBottom: '8px', letterSpacing: '.03em' }}>{c.title}</h4>
                        <p style={{ fontSize: '.82rem', lineHeight: 1.7, fontWeight: 400, color: 'rgba(240,235,232,.45)' }}>{c.desc}</p>
                      </div>
                      <div className="charity-node-top">
                        <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1rem', fontWeight: 300, color: '#cc2200', display: 'block', marginBottom: '8px' }}>{c.year}</span>
                        <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#cc2200', opacity: 0.7, border: '2px solid #0e0e0e', outline: '1px solid rgba(204,34,0,.4)', margin: '0 auto' }} />
                      </div>
                    </>
                  ) : (
                    /* 下排：圆点+年份在上，内容在下 */
                    <>
                      <div className="charity-node-bottom">
                        <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#cc2200', opacity: 0.7, border: '2px solid #0e0e0e', outline: '1px solid rgba(204,34,0,.4)', margin: '0 auto 8px' }} />
                        <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1rem', fontWeight: 300, color: '#cc2200', display: 'block' }}>{c.year}</span>
                      </div>
      ...
      <style>{`
        ...
        .brand-card:hover { border-color: rgba(204,34,0,.4) !important; }
